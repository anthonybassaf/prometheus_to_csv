...
class CSVDataWriter(object):
    ...
    def createNewMetrics(self, metric_from_file, my_date):
        ...
        job = metric_from_file.get('job', None)
        ...
        if "TRIGRAMME" in self.table_name:
            # print(f"Creating new metric with query: {query}\n")
            metric_list.append(MetricTrigramme(name, site, mode, query, availability_zone, aggregates, memory, product_name, job, metric_from_file['trigramme']))
            print("Creating New Metric: ", metric_list[-1].__dict__)
            ...
            if 'loop' in metric_from_file and "TRUE" in metric_from_file['loop']:
                ...
                for num_server in metric_from_file['num_server'].split():
                    ...
                    metric_list.append(MetricServer(name, site, mode, query, availability_zone, aggregates, memory, product_name, job, num_server))
            else:
                metric_list.append(MetricServer(name, site, mode, query, availability_zone, aggregates, memory, product_name, job, metric_from_file['num_server']))
        else:
            metric_list.append(MetricGlobale(name, site, mode, query, availability_zone, aggregates, memory, product_name, job))
        return metric_list
