    ...
    def findAndWriteMetric(self, metric):
        ...
        for result in data['data']['result']:
            ...
            for metric_value in metric_values:
                ...
                metric_name = result['metric'].get('instance_short')
                site = metric.site
                self.writer.writerow(metric.toCSV(my_time, my_value, metric_name, site))
                
     
    def createNewMetrics(self, metric_from_file, my_date):
        name = metric_from_file['name']
        site = metric_from_file['site']
        power_value = metric_from_file['query']      
                
        metric_list = []

        if "TRIGRAMME" in self.table_name:
            metric_list.append(MetricTrigramme(name, site, power_value))
            ...
            if 'loop' in metric_from_file and "TRUE" in metric_from_file['loop']:
                ...
                for num_server in metric_from_file['num_server'].split():
                    ...
                    metric_list.append(MetricServer(name, site, power_value, num_server))
            else:
                metric_list.append(MetricServer(name, site, power_value, metric_from_file['num_server']))
        else:
            metric_list.append(MetricGlobale(name, site, power_value))
