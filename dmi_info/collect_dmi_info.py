    ...
    def findAndWriteMetric(self, metric):
        ...
        for result in data['data']['result']:
            ...
            for metric_value in metric_values:
                ...
                metric_name = result['metric'].get('instance_short')  # Extract the hostname here
                site = metric.site
                product_name = metric.product_name
                self.writer.writerow(metric.toCSV(my_time, my_value, metric_name, site, product_name))
                
     
    def createNewMetrics(self, metric_from_file, my_date):
        name = metric_from_file['name']
        site = metric_from_file['site']
        product_name = metric_from_file['product_name']
        dmi_info_value = metric_from_file['query']      
                
        metric_list = []

        if "TRIGRAMME" in self.table_name:
            metric_list.append(MetricTrigramme(name, site, product_name, dmi_info_value))
            ...
            if 'loop' in metric_from_file and "TRUE" in metric_from_file['loop']:
                ...
                for num_server in metric_from_file['num_server'].split():
                    ...
                    metric_list.append(MetricServer(name, site, product_name, dmi_info_value, num_server))
                ...
                metric_list.append(MetricServer(name, site, product_name, dmi_info_value, metric_from_file['num_server']))
            ...
            metric_list.append(MetricGlobale(name, site, product_name, dmi_info_value))
